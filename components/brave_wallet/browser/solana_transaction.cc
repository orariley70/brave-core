...
SolanaTransaction::SendOptions::SendOptions(
    ...
    : max_retries(max_retries_param),
      preflight_commitment(preflight_commitment_param),
      skip_preflight(skip_preflight_param) {}
...
SolanaTransaction::SendOptions::FromValue(const base::Value& value) {
  ...
  auto max_retries_number = options_dict->FindDouble(kMaxRetries);
  if (max_retries_number && *max_retries_number >= 0 &&
